# Draft AI — Platform Architecture
...
**draft-api** — Primary application server. Handles all API routes, authentication, matching logic, and business rules. Single deployable unit at MVP. Fly.io region: `iad` (Ashburn, VA) for CONUS coverage.

**draft-worker** — Background job processor. Handles async tasks: notification dispatch, deliverable processing, operator webhook management, payment settlement triggers. Runs as a separate Fly.io app so it can scale independently of the API.

**draft-web** — Client-facing frontend. Static SPA served from Fly.io edge or optionally from a CDN (Cloudflare). Talks exclusively to draft-api.
...
**Access control:** Pre-signed URLs generated by draft-api. Files are never publicly accessible. URLs expire after download or a short time window.
...
| Background jobs | BullMQ (Node) or Celery (Python) | Runs on draft-worker Fly.io app |
